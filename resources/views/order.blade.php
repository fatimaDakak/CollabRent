	...
	<div class="container">
		<div class="row">
			...
			<ul class="list-inline dashboard-menu text-center">
					...
					<li><a  class="active" href="{{ url('/order') }}">Objets Demandés</a></li>
					<li><a href="{{ url('/addad') }}">Ajouter Annonce</a></li>
						...
						<table class="table">
							...
							<tbody>
								<tr>
									...
									<td><a href="{{ url('/order') }}"class="btn btn-default">View</a></td>
								</tr>
								<tr>
									<td>#451231</td>
									<td>Mar 25, 2016</td>
			...
			<li><a  href="{{ url('/eval_partenaire') }}">Feedbacks</a></li>
                ...
                <div class="dashboard-wrapper user-dashboard">
                            ...
                            <tbody>
								...
								<tr @if($reservation->isNew() && $reservation->status == 'pending') class="table-warning" @endif>
									...
									<form method="POST" action="{{ route('reservations.update1', $reservation->id) }}">
										...
										<p>Entre  <strong>{{ Auth::user()->name }}</strong>, ci-après dénommé le Client, et <strong></strong>, ci-après dénommé le Partenaire, il est convenu ce qui suit :</p>
                                ...
                                <br/>
								<div id="sig-{{ $reservation->id }}"></div>
								<br/><br/>
								<button id="clear-{{ $reservation->id }}" type="button" class="btn btn-danger btn-sm">Clear</button>
								<textarea id="signature-{{ $reservation->id }}" name="signed" style="display: none"></textarea>
                            <br/>
                            <button id="confirm-button-{{ $reservation->id }}" class="btn btn-primary">Accpeter la demande</button>
					...
					<script>			
						...
						// When the user clicks the confirm button in the signature modal, submit the form
						$('#confirm-button-{{ $reservation->id }}').click(function() {
							var modal = $(this).closest('.modal'); // get the modal containing the clicked confirm button
							var form = modal.find('form'); // get the form inside the modal
							var signature = modal.find('#sig-{{ $reservation->id }}').signature('toJSON'); // get the signature in JSON format
							modal.find('#signature-{{ $reservation->id }}').val(signature); // set the signature value in the hidden input field
							form.submit(); // submit the form
						});
					
						// Initialize the signature pad
						var sig{{ $reservation->id }} = $('#sig-{{ $reservation->id }}').signature({syncField: '#signature-{{ $reservation->id }}', syncFormat: 'PNG'});
						// Clear the signature pad
						$('#clear-{{ $reservation->id }}').click(function (e) {
							e.preventDefault();
							sig{{ $reservation->id }}.signature('clear');
							$("#signature-{{ $reservation->id }}").val('');
						});
